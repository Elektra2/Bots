...
@dp.callback_query_handler(lambda c: c.data.endswith("_sp"),state=FSM_create_test.post_list)
async def post_list(callback_query: types.CallbackQuery, state: FSMContext, n=5):
    ...
    await FSM_create_test.cur_list.set()
...
@dp.callback_query_handler(lambda c: c.data.endswith("_offers"), state=FSM_create_test.cur_list)#чтобы не писать отдельные хэндлеры для каждой функции, у каждой кнопке в каллбэк дате было дописано _offers, что мы здесь и проверяем. А также проверяем что state=cur_list(засетили его при первом вызове функции)
